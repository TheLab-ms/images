...
RUN cp -r /opt/keycloak/providers/*.jar /tmp
...
COPY --from=ptkc /tmp/io.phasetwo.keycloak-keycloak-magic-link-0.13.jar /opt/keycloak/providers
COPY --from=ptkc /tmp/io.phasetwo.keycloak-keycloak-events-0.17.jar /opt/keycloak/providers
